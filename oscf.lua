...
-- # element management
local elements = {}
-- create a default element as a template
elements['default'] = {
    -- z order
    layer = 0,
    ...
    style = {
        ...
        -- back color only works when "osd-back-color" is set in mpv
    ...
    setAlpha = function(self, trans)
            local sta = self.style.alpha or elements['default'].style.alpha
            local alpha = {0, 0, 0, 0}
            self.trans = trans
            for i = 1, 4 do
    ...
    setStyle = function(self)
            local st, std = self.style, elements['default'].style
            local fmt = {'{', '', '', '', '', '', '', '', '}'}
            if st.color then
...
-- add an element of a layout
-- layout: 'idle', 'play', or your newly defined layout 
-- name: name string
-- return: element table
